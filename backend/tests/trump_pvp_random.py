"""Autonomous PvP bot: signs up (or logs in) as 'Trump', then plays live PvP
...
Usage (from backend/, venv python, PYTHONPATH cleared is fine -- no app import):
    env PYTHONPATH= .venv312/bin/python tests/trump_pvp_random.py
"""
import asyncio
import json
import random
...
USERNAME = "Trump"
PASSWORD = "Trump2024!"          # 8+ chars, has letters + a number
DISPLAY = "Trump"
...
def authenticate() -> str:
    """Sign up Trump; if already taken, log in. Returns the session token."""
    try:
        ...
        while True:
            ...
            if t == "waiting":
                ...
                print(f"[game {n}] FINAL: {r['outcome'].upper()} "
                      ...
                      f"Trump now {me.get('rating')} ({me.get('tier')}) "
